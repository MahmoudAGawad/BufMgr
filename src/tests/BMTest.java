...
class BMDriver extends TestDriver implements GlobalConst {
			...
			if (status == OK) {
				...
				if (status == OK) {
					if (data != (pid.pid) + 99999) {
						status = FAIL;
						System.out.println("*** Read wrong data back from page "
								+ pid.pid + "\n");
	...
	protected boolean test2() {
		...
		System.out.printf("Starting Pinning pages starting from pgid = %d\n",firstPid.pid);
	...
	protected boolean test3() {
		...
		if (status == OK) {
			...
			for (index = 0; status == OK && index < numPages; ++index) {
				...
				if (status == OK) {
					try {
						SystemDefs.JavabaseBM.unpinPage(pid, true, false); // might
																			// not
																			// be
																			// dirty
					} catch (Exception e) {
